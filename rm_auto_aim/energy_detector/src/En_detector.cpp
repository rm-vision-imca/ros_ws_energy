...
namespace rm_auto_aim
{
  ...
  En_Detector::En_Detector(float NMS_THRESHOLD, float CONF_THRESHOLD, int detect_color, int binary_thres,int R_area_MIN,int R_area_MAX) : \
  NMS_THRESHOLD(NMS_THRESHOLD), CONF_THRESHOLD(CONF_THRESHOLD), detect_color(detect_color), binary_thres(binary_thres),R_area_MIN(R_area_MIN),R_area_MAX(R_area_MAX)
  {
  ...
  std::vector<Leaf> En_Detector::work(cv::Mat src_img)
  {
    ...
    for (int i = 0; i < output_buffer.rows; i++)
    {
      ...
      else
      {
        ...
        class_ids.emplace_back(0); //{0:"leaf"}// debug leafs
    ...
    for (auto &leaf : leafs)
    {
      ...
      for (size_t i = 0; i < leaf.kpt.size(); i++)
      {
        if (
            ...
            leaf.kpt[i].y > MAX_HEIGHT or R_Point.x < 0 or R_Point.y < 0 or 
  ...
  cv::Point2f En_Detector::findR(cv::Mat src)
  {
    ...
    this->detect_color == BLUE ? cv::threshold(channels[2] * 1.2 - channels[0], bin, binary_thres, 255, cv::THRESH_BINARY) : cv::threshold(channels[0] * 0.9 - channels[2], bin, 100, 255, cv::THRESH_BINARY);
    ...
    if (contours.size() > 0)
    {
      ...
      {
        if (cv::contourArea(contours[i]) >= R_area_MIN&&contourArea(contours[i])<=R_area_MAX)
        {
