...
namespace rm_auto_aim
{
      ...
      : Node("energy_detector", options)
  {
    ...
    RCLCPP_INFO(this->get_logger(), "Starting leaf_detector Node");
    // Detector
    ...
    marker_pub_ =
        this->create_publisher<visualization_msgs::msg::MarkerArray>("/energy_detector/marker", 10);
  ...
  cv::Mat EnergyDetector::VideoTest(cv::Mat &img, cv::Mat &bin)
  {
    ...
    {
      // leaf info
      leaf_msg.leaf_center.x = leaf.kpt[4].x;
      leaf_msg.leaf_center.y = leaf.kpt[4].y;
      leaf_msg.leaf_center.z = 0;
  ...
  void EnergyDetector::ImageCallBack(const sensor_msgs::msg::Image::SharedPtr img_msg)
  {
    ...
    if (pnp_solver_ != nullptr)
    {
      ...
      auto_aim_interfaces::msg::Leaf leaf_msg;
      auto_aim_interfaces::msg::DebugLeaf debug_leaf;
      for (const auto &leaf : leafs)
      {
        // leaf info
        leaf_msg.leaf_center.x = leaf.kpt[4].x;
        ...
        debug_leaf.r_center.y = detector_->R_Point.y;
        debug_leaf.left_bottom.x = leaf.kpt[LeafPointType::BOTTOM_LEFT].x;
        debug_leaf.left_bottom.y = leaf.kpt[LeafPointType::BOTTOM_LEFT].y;
        debug_leaf.right_bottom.x = leaf.kpt[LeafPointType::BOTTOM_RIGHT].x;
        debug_leaf.right_bottom.y = leaf.kpt[LeafPointType::BOTTOM_RIGHT].y;
        debug_leaf.right_top.x = leaf.kpt[LeafPointType::TOP_RIGHT].x;
        debug_leaf.right_top.y = leaf.kpt[LeafPointType::TOP_RIGHT].y;
        debug_leaf.left_top.x = leaf.kpt[LeafPointType::TOP_LEFT].x;
        debug_leaf.left_top.y = leaf.kpt[LeafPointType::TOP_LEFT].y;
        debug_leaf.center.x = leaf.kpt[LeafPointType::CENTER_POINT].x;
        debug_leaf.center.y = leaf.kpt[LeafPointType::CENTER_POINT].y;
        cv::Mat rvec, tvec;
  ...
  void EnergyDetector::createDebugPublishers()
  {
    ...
    binary_img_pub_ = image_transport::create_publisher(this, "/detector/leaf_binary_img");
  ...
  std::unique_ptr<En_Detector> EnergyDetector::initDetector()
  {
    ...
    // detect mode
    param_desc.description = "0-NONE, 1-AUTOFIRE 2-SMALLWIN, 3-BIGWIN";
    param_desc.integer_range[0].from_value = 0;
    param_desc.integer_range[0].to_value = 3;
    auto detect_mode = declare_parameter("detect_mode", NONE, param_desc);
    param_desc.integer_range[0].from_value = 20;
    param_desc.integer_range[0].to_value = 10000;
    int R_area_MAX=declare_parameter("R_area_MAX", 1000, param_desc);
    param_desc.integer_range[0].from_value = 20;
    param_desc.integer_range[0].to_value = 10000;
    int R_area_MIN=declare_parameter("R_area_MIN", 200, param_desc);
    
    auto detector = std::make_unique<En_Detector>(nms_threshold, conf_threshold, detect_color, binary_thres,R_area_MIN,R_area_MAX);
    return detector;
  }
  ...
  { // Convert ROS img to cv::Mat
    ...
    detector_->detect_mode = get_parameter("detect_mode").as_int();
     detector_->R_area_MIN=get_parameter("R_area_MIN").as_int();
    detector_->R_area_MAX=get_parameter("R_area_MAX").as_int();
    // if (detector_->detect_mode != 2 ||detector_->detect_mode != 3)
