  ...
  serial_driver_{new drivers::serial_driver::SerialDriver(*owned_ctx_)}
{
  ...
  cam_parameter_client = std::make_shared<rclcpp::AsyncParametersClient>(this, "camera_node");
...
void RMSerialDriver::initParams()
{
  ...
  Energy_parameters.emplace_back(rclcpp::Parameter("gain", 12.0));
  ...
  Armor_parameters.emplace_back(rclcpp::Parameter("exposure_time", 3800));
  Armor_parameters.emplace_back(rclcpp::Parameter("gain", 8.0));
  ...
  while (rclcpp::ok()) {
    try {
      ...
      if (header[0] == 0x5A) {
        ...
        if (crc_ok) {
          if (!initial_set_param_[0] || packet.detect_color != previous_receive_color_) {
            setParam(rclcpp::Parameter("detect_color", packet.detect_color));
            previous_receive_color_ = packet.detect_color;
          }

          if (packet.reset_tracker) {
            resetTracker(packet.vision_mode);
          }
            
          if (!initial_set_param_[1] || packet.vision_mode != previous_receive_tracker_mode_) {
            previous_receive_tracker_mode_ = packet.vision_mode;
            detect_mode_=previous_receive_tracker_mode_;
            resetEnMode(packet.vision_mode,rclcpp::Parameter("detect_mode", packet.vision_mode));
          }
...
{
  if(detect_mode_!=0&&detect_mode_!=1)return;
...
void RMSerialDriver::resetTracker(uint16_t mode)
{
...
{
  //RCLCPP_INFO(get_logger(), "reach!");
  if(msg->yaw==0)
  if(detect_mode_!=2&&detect_mode_!=3)return;
  if(fabs(msg->yaw)>=50 || fabs(msg->pitch)>=50)return;
  try {
    SendPacket packet;
    packet.tracking = false;
    packet.id = 0.0;
    packet.armors_num = 0.0;
    packet.x = msg->pitch;
    packet.y = msg->yaw;
    packet.z = 0.0;
    packet.yaw = 0.0;
    ...
    packet.r2 = 0.0;
    packet.dz = 0.0;
    //RCLCPP_INFO(get_logger(), "yaw:%f pitch:%f!",msg->yaw,msg->pitch);
    crc16::Append_CRC16_Check_Sum(reinterpret_cast<uint8_t *>(&packet), 52U);
    ...
    std_msgs::msg::Float64 latency;
    latency.data = (this->now() - msg->header.stamp).seconds() * 1000.0;
    RCLCPP_DEBUG_STREAM(get_logger(), "Total EN_latency: " + std::to_string(latency.data) + "ms");
    latency_pub_->publish(latency);
  } catch (const std::exception & ex) {
    RCLCPP_ERROR(get_logger(), "Error while sending data: %s", ex.what());
    reopenPort();
...
void RMSerialDriver::resetEnMode(uint16_t mode,const rclcpp::Parameter & param)
{
  RCLCPP_ERROR(get_logger(), "setting mode : %d", mode);
  if (
    !set_param_future_.valid() ||
    set_param_future_.wait_for(std::chrono::seconds(1)) == std::future_status::ready) {
    RCLCPP_INFO(get_logger(), "Setting detect_mode to %ld...", param.as_int());
    set_param_future_ = Endetector_param_client_->set_parameters(
      {param}, [this, param](const ResultFuturePtr & results) {
        for (const auto & result : results.get()) {
          if (!result.successful) {
        ...
        RCLCPP_INFO(get_logger(), "Successfully set detect_mode to %ld!", param.as_int());
        initial_set_param_[1] = true;
      });
    //setCamParam(mode);
  }
}
void RMSerialDriver::setCamParam(uint16_t mode)
{
  ...
  auto future_set =
    cam_parameter_client->set_parameters(mode==2||mode==3 ? Energy_parameters : Armor_parameters);
    if(future_set.wait_for(std::chrono::seconds(0)) == std::future_status::ready){
      for (const auto & result : future_set.get()) {
          if (!result.successful) {
            RCLCPP_ERROR(get_logger(), "Failed to set parameter: %s", result.reason.c_str());
            return;
          }
        }
        RCLCPP_INFO(get_logger(), "Successfully set params");
        initial_set_param_[1] = true;
      }
}
}  // namespace rm_serial_driver
