...
private:
  ...
  void resetTracker(uint16_t mode);

  void sendEnData(auto_aim_interfaces::msg::EnTarget::SharedPtr msg);

  void resetEnMode(uint16_t mode,const rclcpp::Parameter & param);

  void setCamParam(uint16_t mode);
  //params
  ...
  bool initial_set_param_[2] = {false};//0-color 1-mode
  uint8_t previous_receive_color_ = 0;
  uint16_t previous_receive_tracker_mode_=0;
  rclcpp::AsyncParametersClient::SharedPtr detector_param_client_;
  rclcpp::AsyncParametersClient::SharedPtr Endetector_param_client_;
  rclcpp::AsyncParametersClient::SharedPtr mode_param_client_;
